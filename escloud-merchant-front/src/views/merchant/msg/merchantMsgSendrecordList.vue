<template>
  <div class="app-container calendar-list-container">
    <div class="filter-container">
      <el-form :model="filterForm" ref="filterForm">
        <div v-if=proSearch>
          <el-row>
            <el-col :span="8">
              <el-form-item label="id" :label-width="filterFormLabelWidth" prop="id" class="form-bottom">
                <el-input @keyup.enter.native="searchMerchantMsgSendrecord" size="small" class="filter-item" placeholder="id" v-model="filterForm.id">
                </el-input>
              </el-form-item>
            </el-col>
            <el-col :span="8">
              <el-form-item label="商户id" :label-width="filterFormLabelWidth" prop="merchantId" class="form-bottom">
                <el-input @keyup.enter.native="searchMerchantMsgSendrecord" size="small" class="filter-item" placeholder="商户id" v-model="filterForm.merchantId">
                </el-input>
              </el-form-item>
            </el-col>
            <el-col :span="8">
              <el-form-item label="消息类型表id" :label-width="filterFormLabelWidth" prop="msgTypeId" class="form-bottom">
                <el-input @keyup.enter.native="searchMerchantMsgSendrecord" size="small" class="filter-item" placeholder="消息类型表id" v-model="filterForm.msgTypeId">
                </el-input>
              </el-form-item>
            </el-col>
          </el-row>
          <el-row>
            <el-col :span="8">
              <el-form-item label="发送时间" :label-width="filterFormLabelWidth" prop="msgSendDate" class="form-bottom">
                <el-date-picker v-model="filterForm.msgSendDate" type="datetimerange" placeholder="发送时间" range-separator="至" start-placeholder="开始日期" end-placeholder="结束日期" prop="msgSendDate">
                </el-date-picker>
              </el-form-item>
            </el-col>
            <el-col :span="8">
              <el-form-item label="发送情况" :label-width="filterFormLabelWidth" prop="msgSendStatus" class="form-bottom">
                <el-select @keyup.enter.native="searchMerchantMsgSendrecord" size="small" class="filter-item" clearable placeholder="请选择" v-model="filterForm.msgSendStatus">
                  <el-option v-for="item in msgSendStatusMapArr" :key="item.value" :label="item.label" :value="item.value">
                  </el-option>
                </el-select>
              </el-form-item>
            </el-col>
          ...
          <el-row>
            <el-col :span="8">
              <el-form-item label="创建人" :label-width="filterFormLabelWidth" prop="createBy" class="form-bottom">
                <el-input @keyup.enter.native="searchMerchantMsgSendrecord" size="small" class="filter-item" placeholder="创建人" v-model="filterForm.createBy">
                </el-input>
              </el-form-item>
            </el-col>
            <el-col :span="8">
            ...
            <el-col :span="8">
              <el-form-item label="更新人" :label-width="filterFormLabelWidth" prop="updateBy" class="form-bottom">
                <el-input @keyup.enter.native="searchMerchantMsgSendrecord" size="small" class="filter-item" placeholder="更新人" v-model="filterForm.updateBy">
                </el-input>
              </el-form-item>
            </el-col>
          </el-row>
          <el-row class="button-group">
            <el-col :span="12" class="searchSimpleBtn">
              <el-form-item label=" " :label-width="filterFormLabelWidth" class="form-bottom">
                <el-button type="primary" size="small" icon="search" @click="searchMerchantMsgSendrecord" v-waves>搜索</el-button>
      ...
      <el-form>
        ...
        <el-col :span="3">
          <el-form-item label=" " :label-width="filterFormLabelWidth2" prop="queryType" class="simple-btn first">
            <el-select @keyup.enter.native="searchMerchantMsgSendrecord" size="small" class="filter-item" @change="searchName" clearable placeholder="请选择查询条件" v-model="queryType">
        ...
        <el-col :span="3">
          <el-form-item label=" " :label-width="filterFormLabelWidth2" prop="queryParams" class="simple-btn">
            <el-input @keyup.enter.native="searchMerchantMsgSendrecord" size="small" class="filter-item" :placeholder="selectDesc" v-model="queryParams">
            </el-input>
          </el-form-item>
        </el-col>
    ...
    <el-table ref="multipleTable" :key='tableKey' :data="list" stripe fit highlight-current-row size='mini' :default-sort="{prop: 'id', order: 'aescending'}" @select="selectCheckBox" @select-all="selectAllCheckBox" v-loading="listLoading" element-loading-text="给我一点时间" style="width: 100%">
      ...
      <el-table-column header-align="center" align="center" label="发送时间" prop='msgSendDate'>
        <template slot-scope="scope">
          <span>{{scope.row.msgSendDate}}</span>
        </template>
      </el-table-column>
      <el-table-column header-align="center" align="center" label="发送情况" prop='msgSendStatus' :show-overflow-tooltip='true'>
        <template slot-scope="scope">
          <span>{{msgSendStatusMapObj[scope.row.msgSendStatus]}}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column align="center" label="操作" width="240" fixed='right'>
        <template slot-scope="scope">
          <el-button size="small" type="text" @click="modifyMerchantMsgSendrecord(scope.row,'update')">修改
          </el-button>
          <el-button size="small" type="text" @click="deleteMerchantMsgSendrecord(scope.row,'delete')">删除
          </el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <div v-show="!listLoading" class="pagination-container">
      <el-row>
        <el-col :span="7">
          <el-button type="info" plain size="small" icon="edit" @click="addMerchantMsgSendrecord">添加</el-button>
    ...
    <el-dialog class="create_dialog" :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible" size="small" :before-close="closeDialog">
      <el-form :model="merchantMsgSendrecordForm" :rules="merchantMsgSendrecordFormRules" ref="merchantMsgSendrecordForm" label-width="80px">
        <el-row>
          <el-col :span="11">
            <el-form-item v-if="dialogStatus=='update'" label="id" :label-width="formLabelWidth" prop="id">
              <el-input size="small" v-model="merchantMsgSendrecordForm.id" :disabled="true"></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="11">
            <el-form-item label="商户id" :label-width="formLabelWidth" prop="merchantId">
              <el-input size="small" v-model="merchantMsgSendrecordForm.merchantId" :maxlength="32"></el-input>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
          <el-col :span="11">
            <el-form-item label="消息类型表id" :label-width="formLabelWidth" prop="msgTypeId">
              <el-input size="small" v-model="merchantMsgSendrecordForm.msgTypeId" :maxlength="11"></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="11">
            <el-form-item label="发送时间" :label-width="formLabelWidth" prop="msgSendDate">
              <el-date-picker v-model="merchantMsgSendrecordForm.msgSendDate" type="datetime" placeholder="选择日期时间"></el-date-picker>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
          <el-col :span="11">
            <el-form-item label="发送情况" :label-width="formLabelWidth" prop="msgSendStatus">
              <el-select size="small" v-model="merchantMsgSendrecordForm.msgSendStatus" clearable placeholder="请选择">
                <el-option v-for="item in msgSendStatusMapArr" :key="item.value" :label="item.label" :value="item.value">
                </el-option>
              </el-select>
            </el-form-item>
          ...
          <el-col :span="11">
            <el-form-item label="更新日期" :label-width="formLabelWidth" prop="updateDate">
              <el-date-picker v-model="merchantMsgSendrecordForm.updateDate" type="datetime" placeholder="选择日期时间"></el-date-picker>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
          <el-col :span="11">
            <el-form-item label="更新人" :label-width="formLabelWidth" prop="updateBy">
              <el-input size="small" v-model="merchantMsgSendrecordForm.updateBy" :maxlength="20"></el-input>
            </el-form-item>
          </el-col>
        </el-row>
      </el-form>
...
<script>
  import { getMerchantMsgSendrecordList, addMerchantMsgSendrecord, editMerchantMsgSendrecord, delMerchantMsgSendrecord, delBatchMerchantMsgSendrecord } from '@/api/merchant/msg/merchantMsgSendrecord'
  ...
  export default {
    name: 'tableMerchantMsgSendrecord',
    directives: {
      waves
    },
    ...
    data() {
      return {
        ...
        msgSendStatusMapObj: {},
        // 表单校验规则
        merchantMsgSendrecordFormRules: {
          id: [{
            type: 'number',
            required: true,
            message: '请输入id',
    ...
    created() {
      ...
      this.getMerchantMsgSendrecordList()
    },
    methods: {
      getMerchantMsgSendrecordList() {
        this.listLoading = true
        this.filterForm.curApplicationCode = this.curApplicationCode
        getMerchantMsgSendrecordList(this.filterForm).then(response => {
      ...
      addMerchantMsgSendrecord() {
        this.dialogStatus = 'create'
        this.dialogFormVisible = true
      },
      modifyMerchantMsgSendrecord(row) {
        this.merchantMsgSendrecordForm = Object.assign({}, row)
        this.dialogStatus = 'update'
        this.dialogFormVisible = true
      },
      deleteMerchantMsgSendrecord(row) {
        const param = {
          primaryKey: row.id,
          curApplicationCode: this.curApplicationCode
        }
        this.listLoading = true
        delMerchantMsgSendrecord(param).then(response => {
          if (response.data.error_no === 0) {
            this.$notify({
              title: '成功',
              message: '删除成功',
            ...
            this.searchMerchantMsgSendrecord()
      ...
      deleteBatch() {
        ...
        this.listLoading = true
        delBatchMerchantMsgSendrecord(param).then(response => {
          if (response.data.error_no === 0) {
            this.$notify({
              title: '成功',
              message: '删除成功',
      ...
      addSubmit() {
        this.$refs['merchantMsgSendrecordForm'].validate((valid) => {
          if (valid) {
            this.listLoading = true
            this.merchantMsgTypeForm.curApplicationCode = this.curApplicationCode
            addMerchantMsgSendrecord(this.merchantMsgSendrecordForm).then(response => {
              if (response.data.error_no === 0) {
      ...
      updateSubmit() {
        ...
        this.$refs['merchantMsgSendrecordForm'].validate((valid) => {
          if (valid) {
            this.listLoading = true
            editMerchantMsgSendrecord(this.merchantMsgSendrecordForm).then(response => {
              if (response.data.error_no === 0) {
                this.dialogFormVisible = false
                this.$notify({
                  title: '成功',
      ...
      resetForm() {
        this.merchantMsgSendrecordForm = {
          id: '',
          merchantId: '',
          msgTypeId: '',
          msgSendDate: '',
        ...
        if (this.$refs['merchantMsgSendrecordForm'] !== undefined) {
          this.$refs['merchantMsgSendrecordForm'].resetFields()
        }
      },
      handleDownload() {
        ...
        require.ensure([], () => {
          ...
          export_json_to_excel(tHeader, data, '消息发送记录表')
